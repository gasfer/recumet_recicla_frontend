...
export class QueryInputsComponent implements OnInit {
  ...
  async showTraceability(input: Input) {
    ...
    try {
      ...
      return;
      const events = response.traceability.events || [];
      const html = events.length ? events.map((event: any) => {
        ...
        return `<article style="border-left:4px solid #0ea5e9;padding:.65rem .9rem;margin:.6rem 0;background:#f8fafc;text-align:left;border-radius:6px"><div style="display:flex;justify-content:space-between;gap:1rem;flex-wrap:wrap"><b>${this.escapeHtml(event.event_type)}</b><span>${this.escapeHtml(new Date(event.createdAt).toLocaleString())}</span></div><div><b>Realizó:</b> ${this.escapeHtml(event.actor?.full_names || 'Usuario no disponible')}</div>${event.authorizer ? `<div><b>Autorizó:</b> ${this.escapeHtml(event.authorizer.full_names)}</div>` : ''}${event.reason ? `<div><b>Motivo:</b> ${this.escapeHtml(event.reason)}</div>` : ''}${event.historical_incomplete ? '<div style="color:#b45309"><b>Antecedente general:</b> no existe detalle anterior disponible.</div>' : ''}${changes ? `<ul style="margin:.4rem 0 0;padding-left:1.2rem">${changes}</ul>` : ''}</article>`;
    ...
    } catch { await Swal.fire('Atención', 'No se pudo consultar la trazabilidad de la compra.', 'error'); }
  }
  ...
  async anularInput(input: Input) {
    try {
      ...
      icon: `warning`,
      input: 'textarea', inputLabel: 'Motivo obligatorio', inputPlaceholder: 'Explique por qué se anula esta compra',
    ...
    } catch { await Swal.fire('Atención', 'No se pudo preparar la anulación de la compra.', 'error'); }
  }
