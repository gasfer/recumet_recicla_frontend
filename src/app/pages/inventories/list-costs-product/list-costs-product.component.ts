...
export class ListCostsProductComponent implements OnInit, OnDestroy {
...
private pdfSub?:   Subscription;
  ...
  getAllCategories() {
    ...
    this.categoriesService.getAllAndSearch(1, 10000, true).subscribe(resp => {
      this.categories.set(
        resp.categories.data.map(category => ({
          name: category.name,
  ...
  getAllAndSearchProducts(page: number, limit: number, type: string = '', query: string = '') {
    ...
    // ✅ Cancela suscripción anterior — evita duplicados
    this.productsSub?.unsubscribe();

    this.productsSub = this.productsService
      .getAllAndSearchProductCosts(page, limit, id_category, type, query, this.fieldSort(), this.order())
      .subscribe({
        next:     (resp) => { this.productsCosts.set(resp.products); },
  ...
  saveProductCostUpdate(row: any) {
    this.productsService.postNewProductCost(row).subscribe({
      complete: () => { this.errorLogsService.notifications('Modificado correctamente', 'success'); },
      error:    () => this.loading.set(false)
    });
  }
  ...
  clearInputs() {
    this.formReport.patchValue({ id_category: '' });
  }
...
printExcelReport() {
  ...
  Swal.fire({
    ...
    didOpen: () => {
      Swal.showLoading();
      this.excelSub?.unsubscribe(); // ✅ cancela anterior
      this.excelSub = this.productsService
        .getReportProductCostExcel(id_category, this.fieldSort(), this.order())
        .subscribe({
...
printPdfReport() {
  ...
  Swal.fire({
    ...
    didOpen: () => {
      Swal.showLoading();
      this.pdfSub?.unsubscribe(); // ✅ cancela anterior
      this.pdfSub = this.productsService
        .getReportProductCostPdf(id_category, this.fieldSort(), this.order())
        .subscribe({
          next:  (data) => { window.open(window.URL.createObjectURL(data)); Swal.close(); },
          error: ()     => Swal.close()
        });
