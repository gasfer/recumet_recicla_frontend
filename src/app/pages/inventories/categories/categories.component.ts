...
import { Categories, Category, CategoryType } from '../interfaces/categories.interface';
...
export class CategoriesComponent implements OnInit, OnDestroy {
  ...
  cols = signal<ColsTable[]>([
    ...
    {
      field: `type`, header: 'TIPO', style: 'min-width:200px;', tooltip: true, isValueUpdate: true,
      tagValue: (val: string) => {
        switch (val) {
  ...
  getAllAndSearchCategories(page: number, limit: number, status: boolean, type: string = '', query: string = '') {
    ...
    this.categoriesService.getAllAndSearch(page, limit, status, type, query, this.categoryTypeFilter(), this.fieldSort(), this.order()).subscribe({
      next: (resp) => {
